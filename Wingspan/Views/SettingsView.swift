...
struct SettingsView: View {
    ...
    var body: some View {
        ...
        HStack {
            ...
            Slider(value: $config.segmentDuration, in: 0...10, step: 1).frame(maxWidth: 250).onChange(of: config.segmentDuration) { newValue in
                withAnimation {
                    isRealtime = newValue == UserStreamConfiguration.kRealtime
                }
            }
            
            Text(config.segmentDuration == UserStreamConfiguration.kRealtime ? "Realtime" : String(format: "%.0f", config.segmentDuration) + " s")
        ...
        if !isRealtime {
            HStack {
                ...
                Text(config.videoBitrateMbps == UserStreamConfiguration.kLossless ? "Lossless" : (String(format: "%.1f", config.videoBitrateMbps) + " Mbps"))
                    .frame(width: 80, alignment: .trailing)
        ...
        HStack {
            ...
            Spacer()
            Button("Tap to Reset") {
                config = UserStreamConfiguration()
            }
        }.padding().background(Color(UIColor.secondarySystemBackground)).cornerRadius(8)
